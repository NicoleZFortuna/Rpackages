...
Hadley Wickham. Let’s start by making sure that these packages have been
installed.
...
project. Either click on **File-\>New Project**, or the
<img src="pics/ProjectButton.png" width="20"> button. A ‘Create Project’
window will pop up, and you should select **New Directory-\>R Package**.
The following window will then open, allowing you to select a name for
your package, as well as choose the directory in which the package will
be stored. At this point, you can select if you would like to initiate
this project with a Git repository. If you already have Git installed on
your computer, I recommend that you check the **Create a git
repository** box before you select **Create Project**.

<center>
<img src="pics/ProjectPrompt.png" width="400">
</center>

A new RStudio window will open containing your new project. In your
project window, you will see that a number of files have been
...
line (supplying your own directory and name).
...
If you have Git installed on your computer, you can turn this project
into a git repository as well by executing the following line:
...
R packages must be organised with a particular file structure. Usefully,
RStudio and the `usethis` package have functionality to help you get
this right. Some files and folders are more or less necessary, but you
...
A DESCRIPTION file will have automatically generated for you when you
generated your project. Now you must fill in this file in order to
generate appropriate metadata for the package. The first thing we should
do is specify the license, because it’s annoying and we do not want to
think about it ever again.

> ***Note:*** To be clear, you do not need to specify a license for your
...
document how they have changed the code if they choose to share
modifications. If your package is built by a community or is owned by an
organisation, you will need to take more care to choose the correct
licence for your situation.

To easily add the recommended license for your package, run the
following code:
...
Filling the DESCRIPTION file will require some degree of manual
addition. You will need to provide the Title, Author (yourself),
...
in the workshop. As you use these functions while developing your
package, they will update the DESCRIPTION file.
...
to find assistance). Much of the work for building help files has been
made easier with the `roxygen2` and `devtools` packages.
...
functions. This information should not be treated as R functions as they
are not themselves functions. To accommodate the data, you will need to
generate two new folders. A data folder which will contain the built
data objects, as well as a data_raw file which will contain scripts to
generate the data contained in the data folder.
...
Now what you need to do is add some documentation for the data. This
...
documentation. For each data object that you make available to your
...
Note that each line of the documentation begins with `#'`. For an
...
Wickham. It will also give some examples of how to improve your
description of the format of your object. Do not be too concerned with
documenting the `tarotAnswers` object in great detail for this workshop.
...
functions into each of these files according to the theme of the file.
...
<img src="pics/NewScript.png" width="40" /> **-\>R Script**. In either
case, you will need to give the script an appropriate name so that you
know what type of functions are inside, and then save the file in your R
folder.

The easier, automatic way to do this is to use the following function
from the `usethis` package, where you pass the name that you want to
give the R script as an argument:

``` r
use_r("scriptName")
```

Once you have finished placing functions into files, you should delete
the hello.R script.
...
The syntax for documenting functions is similar to that used for
documenting data.
...
This is required for the `roxygen2` (via the `document()` function) to
recognise that this information should be used to generate help files.
...
Importing functions that your function requires (by including
`@importFrom` in your documentation) is useful as it makes sure that
those functions have been exported by your package without having to
load the entire supporting package into the environment.
...
If you want to use a code chunk in your README to demonstrate how to use
a function (including the output of that function), then don’t include
`eval=FALSE` argument.
...
Before you push a vignette (or README) to your GitHub, you must first
<img src="pics/Knit.png" width="50"> the file to generate the html
document that will be viewable on GitHub.
...
functionality. In the top right pane, you should see a **Git** tab, if
you click on it you will be given a number of tools that will allow you
to interact with Git. Along the top of this tab are a number of buttons
...
the user simply needs to tick the boxes listed in the staged column. As
shown below.

<center>
<img src="pics/GitTab2.png" width="400">
</center>
...
In the Git tab, it is possible to pull from the remote repository (blue
down button), and push to the remote repository (green up button). You
can view the previous commit messages by clicking the clock. The blue
gear button provides functionality to **Revert…** to previous commits,
as well as to modify the **Ignore…** (.gitignore) file. The button with
purple shapes allows the user to create new branches, as well as add new
remote repositories. Finally, you can see the name of the branch
...
If Git on your computer has already been set up to interact with your
GitHub account, you will be able to automatically generate a remote
...
[Licensing your
package](https://thinkr-open.github.io/licensing-r/whatis.html)
